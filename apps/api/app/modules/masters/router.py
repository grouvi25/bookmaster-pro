...
# ── Link Page (TapLink) — theme config ─────────────────────
...
async def get_my_page(
    ...
    """Получить настройки публичной страницы мастера (для редактора)."""
    from app.modules.masters.models import MasterPage
    ...
    result = await db.execute(
        select(MasterPage).where(MasterPage.master_id == master.id)
    )
    page = result.scalar_one_or_none()

    if not page:
        # Создаём дефолтную страницу
        page = MasterPage(master_id=master.id)
        db.add(page)
        await db.commit()
    ...
    return {
        ...
        "theme": page.theme,
        "theme_config": page.theme_config or {},
        "custom_links": page.custom_links or [],
        "cover_image_url": page.cover_image_url,
        "show_reviews": page.show_reviews,
        "show_portfolio": page.show_portfolio,
        "show_services": page.show_services,
        "show_prices": page.show_prices,
...
@router.patch("/me/page")
async def update_my_page(
    body: dict,
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Обновить настройки публичной страницы мастера (из редактора).

    Принимает любое подмножество полей:
    theme, theme_config, custom_links, cover_image_url,
    show_reviews, show_portfolio, show_services, show_prices,
    bio_text, seo_title, seo_description.
    """
    from app.modules.masters.models import MasterPage
